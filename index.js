...
*/
function getArtistByIndex(arreh, indx){
      return `the artist at index ${arreh[indx].id} is ${arreh[indx].name}`;
    }
...
function lotsOfArt(theArray){
  let paintArray = [];
  for(let i in theArray){
  if(theArray[i].paintings >= 100){
    paintArray.push(theArray[i].name);
  }
}
  return paintArray;
}
